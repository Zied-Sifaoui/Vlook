...
def draw_scurve_brow(frame, upper_pts, lower_pts, brow_color):
    ...
    alpha3 = np.stack([combined * 0.80] * 3, axis=-1)
    ...
    if result.face_landmarks:
        for face_landmarks in result.face_landmarks:
            ...
            for upper_idx, lower_idx in [
                ...
                frame = draw_scurve_brow(frame, upper_pts, lower_pts, DARK_BROWN)

    cv2.putText(frame, "Style: S-Curve (Bold Fashion) | Dark Brown", (10, 28),
                cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 255, 255), 1, cv2.LINE_AA)
    cv2.imshow("S-Curve Brow - Dark Brown", frame)
