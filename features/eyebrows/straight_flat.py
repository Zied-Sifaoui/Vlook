...
def draw_straight_brow(frame, upper_pts, lower_pts, brow_color):
    ...
    # ── STRAIGHT: flatten both edges to their mean Y ──────────────────────────
    ...
    lower_flat[:, 1] = mean_upper_y + brow_gap * 0.85  # slightly thinner
    ...
    if result.face_landmarks:
        for face_landmarks in result.face_landmarks:
            ...
            for upper_idx, lower_idx in [
                ...
                frame = draw_straight_brow(frame, upper_pts, lower_pts, brow_color)

    cv2.putText(frame, "Style: Straight & Flat (Korean)", (10, 28),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1, cv2.LINE_AA)
    cv2.imshow("Straight Brow", frame)
