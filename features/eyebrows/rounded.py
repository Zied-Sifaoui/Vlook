...
def draw_rounded_brow(frame, upper_pts, lower_pts, brow_color):
    ...
    # ── ROUNDED: uniform dome lift across full width, peak at 40% (not center) ─
    t = np.linspace(0, 1, n)
    round_lift = np.exp(-((t - 0.42) ** 2) / (2 * 0.18 ** 2))  # gaussian dome
    ...
    # Uniform thickness (rounded brows don't taper as aggressively)
...
while cap.isOpened():
    ...
    cv2.putText(frame, "Style: Rounded (Soft/Feminine)", (10, 28),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1, cv2.LINE_AA)
    cv2.imshow("Rounded Brow", frame)
