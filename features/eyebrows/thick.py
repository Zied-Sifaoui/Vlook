...
def draw_thick_brow(frame, upper_pts, lower_pts, brow_color):
    ...
    # ── THICK: expand brow upward AND downward, gentle arch ───────────────────
    t = np.linspace(0, 1, n)
    ...
    # ── Three layers: outer soft glow + mid fill + dense core ─────────────────
    ...
    if result.face_landmarks:
        for face_landmarks in result.face_landmarks:
            ...
            for upper_idx, lower_idx in [
                ...
                frame = draw_thick_brow(frame, upper_pts, lower_pts, brow_color)

    cv2.putText(frame, "Style: Thick & Bushy (Natural Full)", (10, 28),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1, cv2.LINE_AA)
    cv2.imshow("Thick Brow", frame)
