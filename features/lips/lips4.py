...
# ==========================================
# 💋 Upper Lip ONLY Enlargement
# ==========================================
def enlarge_upper_lip(frame, landmarks, strength=0.4):
    h, w = frame.shape[:2]

    # Upper lip landmarks only
...
def main():
    ...
    while True:
        ...
        if results.face_landmarks:
            frame = enlarge_upper_lip(frame, results.face_landmarks[0], strength)

        cv2.putText(frame, f"Upper Strength: {strength:.2f}", (10, 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)

        cv2.imshow("Upper Lip Enlargement", frame)
