...
def _make_proj(K: np.ndarray, W: int, H: int, near: float = 1.0, far: float = 200000.0) -> np.ndarray:
...
class ARRenderer:
    def __init__(self, obj_path: str, normalize_span: float = 4.0):
        verts_raw, self.faces = self._load_obj(obj_path)
        
        # ✅ PROPER AXIS ALIGNMENT (Y-up OBJ → OpenCV Y-down, Z-forward)
        # Rotates mesh 180° around X-axis so local axes match camera space
        R_fix = np.array([
            [1,  0,  0],
            [0, -1,  0],
            [0,  0, -1]
        ], dtype=np.float32)
        verts_raw = (R_fix @ verts_raw.T).T
        ...
        # Center X for natural anchor
        x_center = (centered[:, 0].max() + centered[:, 0].min()) / 2.0
        centered[:, 0] -= x_center
        ...
        print(f"[AR] {obj_path}  verts={len(verts_raw)} faces={len(self.faces)} "
              f"backend={'OpenGL/GPU' if _HAS_MGL else 'CPU'}")
    ...
    def _build(self, scale: float, ox: float, oy: float) -> np.ndarray:
        v = self._verts_norm.copy()
        v = v * scale
        v[:, 0] += ox
        v[:, 1] += oy
        return v
    ...
    def _init_gl(self, W: int, H: int):
        ...
        self._fbo = self._ctx.framebuffer(
            color_attachments=[self._col_tex],
            depth_attachment=self._dep_rb)
        self._fd_tex = self._ctx.texture((W, H), 1, dtype="f4")
        self._fd_tex.filter = moderngl.NEAREST, moderngl.NEAREST
        self._W, self._H = W, H
        print(f"[AR] OpenGL ready {W}x{H} GPU={self._ctx.info.get('GL_RENDERER', 'unknown')}")

    def _make_vbo(self, verts_cam: np.ndarray, normals_cam: np.ndarray) -> np.ndarray:
    ...
    def _render_gpu(self, frame: np.ndarray, verts_cam: np.ndarray,
                    normals_cam: np.ndarray, hair_color: np.ndarray,
                    opacity: float, K: np.ndarray,
                    face_depth_map: Optional[np.ndarray], occ_margin: float) -> np.ndarray:
        H, W = frame.shape[:2]
        ...
        result = (rgba[:, :, :3].astype(np.float32) * a + frame.astype(np.float32) * (1.0 - a))
        vao.release()
        vbo.release()
        return result.astype(np.uint8)

    def _render_cpu(self, frame, pts2d, avg_z, brightness, hair_color, opacity, face_depth_map, occ_margin):
        ...
        margin = max(H, W) * 2
        max_c = np.maximum(np.abs(p0).max(1), np.maximum(np.abs(p1).max(1), np.abs(p2).max(1)))
        ...
        a = ab[:, :, None]
        return (overlay.astype(np.float32) * a + frame.astype(np.float32) * (1.0 - a)).astype(np.uint8)

    def render(self, frame: np.ndarray, rvec, tvec, K, style=None,
               offset_x: float = 0.0, offset_y: float = 0.0, offset_z: float = 0.0,
               mesh_scale: float = 450.0, opacity: float = 0.92,
               face_depth_map: Optional[np.ndarray] = None, occ_margin: float = 15.0, **kwargs) -> np.ndarray:
        hair_color = np.array(style["frame"] if style else [40, 25, 15], np.float32)

        R, _ = cv2.Rodrigues(rvec)
        tvec_shifted = (tvec.reshape(3) + R[:, 2] * offset_z).reshape(3, 1)

        verts_obj = self._build(mesh_scale, offset_x, offset_y)
        verts_cam = (R @ verts_obj.T).T + tvec_shifted.reshape(1, 3)
        normals_cam = (R @ self._face_normals.T).T

        if _HAS_MGL:
            return self._render_gpu(frame, verts_cam, normals_cam, hair_color, opacity, K, face_depth_map, occ_margin)

        dist = np.zeros((4, 1), np.float32)
        pts2d, _ = cv2.projectPoints(verts_obj, rvec, tvec_shifted, K, dist)
        ...
        return self._render_cpu(frame, pts2d, avg_z, bright, hair_color, opacity, face_depth_map, occ_margin)
