...
# ── Per-vertex FADE SCORE (tip → hinge direction) ─────────────────────────────
# Temple tip  Z ≈ +0.117 (deepest into screen) → fades FIRST  (score=1)
# Front frame Z ≈ -0.024 (hinge)               → fades LAST   (score=0)
# The fade sweeps diagonally: tip disappears first, eye-end last.
Z_HINGE = float(verts_raw[:,2].min())   # ≈ -0.030
Z_TIP   = float(verts_raw[:,2].max())   # ≈ +0.117
Z_RANGE = Z_TIP - Z_HINGE              # ≈  0.147

# 0=hinge end, 1=tip — pure Z because arm runs 97% along Z axis
V_Z_SCORE = np.clip((verts_raw[:,2] - Z_HINGE) / (Z_RANGE + 1e-8),
                    0.0, 1.0).astype(np.float32)

# Which arm: +1=right, -1=left, 0=front frame
ARM_THRESH = half_w * 0.55
V_SIDE = np.where(verts_raw[:,0] >  ARM_THRESH,  1.0,
         np.where(verts_raw[:,0] < -ARM_THRESH, -1.0, 0.0)).astype(np.float32)

# Fade weight = Z_score only on arm verts; frame verts are always 0
V_FADE_WEIGHT = (V_Z_SCORE * (np.abs(V_SIDE) > 0.5)).astype(np.float32)
print(f"Fade axis: Z_hinge={Z_HINGE:.4f} Z_tip={Z_TIP:.4f}  "
      f"right={( V_SIDE> 0.5).sum()} left={( V_SIDE<-0.5).sum()} frame={(np.abs(V_SIDE)<0.5).sum()}")
...
def nothing(x): pass
cv2.createTrackbar('Fade start',  WINDOW,  8, 60,  nothing)  # deg before fade begins
cv2.createTrackbar('Fade end',    WINDOW, 35, 90,  nothing)  # deg fully hidden
cv2.createTrackbar('Wave sharp',  WINDOW, 8,  20,  nothing)  # wavefront sharpness
...
# ─────────────────────────────────────────────────────────────────────────────
# 4. PER-VERTEX ALPHA  — diagonal tip-to-hinge fade
# ─────────────────────────────────────────────────────────────────────────────
def vertex_alpha(yaw, fade_start, fade_end, wave_sharpness):
    """
    Computes per-vertex alpha so the fade sweeps diagonally:
      - Tip of temple arm disappears first  (V_FADE_WEIGHT = 1)
      - Hinge / front frame disappears last (V_FADE_WEIGHT = 0)

    'wave_sharpness' controls the width of the dissolve wavefront:
      low  = wide soft gradient (whole arm fades slowly together)
      high = narrow sharp wave  (crisp sweep from tip to hinge)

    yaw > 0 → turning right → LEFT  arm hides (V_SIDE = -1)
    yaw < 0 → turning left  → RIGHT arm hides (V_SIDE = +1)
    """
    abs_yaw = abs(yaw)
    if abs_yaw < fade_start:
        return np.ones(len(V_FADE_WEIGHT), dtype=np.float32)

    # 0→1 as yaw goes from fade_start to fade_end
    yaw_t = float(np.clip(
        (abs_yaw - fade_start) / max(fade_end - fade_start, 1e-6),
        0.0, 1.0))

    # Which arm is hiding?
    # yaw>0 (turning right) → left arm (V_SIDE=-1) hides → hide_side=-1
    hide_side = float(-np.sign(yaw)) if yaw != 0 else 0.0

    # Per-vertex "is this vertex on the hiding arm?"
    # 1.0 if on the correct arm, 0.0 otherwise (other arm + frame never affected)
    on_hiding_arm = np.clip(V_SIDE * hide_side, 0.0, 1.0)  # (V,)

    # The fade wavefront position advances from 1→0 as yaw_t goes 0→1
    # At yaw_t=0: wavefront at 1.0 → nothing faded yet
    # At yaw_t=1: wavefront at 0.0 → entire arm faded
    wavefront = 1.0 - yaw_t   # position along arm where fade is happening

    # Per-vertex fade amount:
    #   vertices with fade_weight > wavefront are ahead of the wave → hidden
    #   vertices with fade_weight < wavefront are behind the wave   → visible
    # wave_sharpness controls transition width (higher = sharper edge)
    fade = np.clip(
        (V_FADE_WEIGHT - wavefront) * wave_sharpness + 0.5,
        0.0, 1.0
    ) * on_hiding_arm   # only affects the hiding arm

    return np.clip(1.0 - fade, 0.0, 1.0).astype(np.float32)
...
while True:
    ...
    wave_sharp  = float(max(cv2.getTrackbarPos('Wave sharp', WINDOW), 1))
    ...
    v_alph  = vertex_alpha(yaw, fade_start, fade_end, wave_sharp)
    ...
    cv2.putText(frame,
        ...
        f"wave:{wave_sharp:.0f}  blur:{blur_k}",
